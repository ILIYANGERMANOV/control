...
public class MessageAdapter extends ResourceCursorAdapter {
    ...
    boolean mWrongPasswordForFirstTime = true;

    public void setFrom(String from) {
        mFrom = from;
    }

    public MessageAdapter(Context context, Cursor c, String password, FragmentActivity activity) {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        if (mFrom != null) {
            MyDebugger.log("mFrom", mFrom);
            if(!MyUtils.parseContactName(mFrom).equals(Constants.OPERATOR)) {
                MCrypt mCrypt = new MCrypt(mPassword);
                String decrypted = mCrypt.decrypt(subject);
                if(decrypted != null) {
                    if(!decrypted.equals(Constants.ENCRYPTION_STATUS_WRONG_PASS)) {
                        MyUtils.resetPasswordTries(context);
                        subject = decrypted;
                    } else {
                        MyDebugger.log("adapter", "WRONG PASSWORD");
                        if(mWrongPasswordForFirstTime) {
                            Intent resultIntent = new Intent();
                            resultIntent.putExtras(mBundle);
                            mActivity.setResult(Activity.RESULT_OK, resultIntent);
                            mActivity.finish();
                            mWrongPasswordForFirstTime = false;
                        }
                    }
                } else {
                    MyDebugger.log("DECRYPTED IS NULL");
                    if(mWrongPasswordForFirstTime) {
                        mActivity.finish();
                        mWrongPasswordForFirstTime = false;
                    }
            ...
            MyDebugger.log("MessageAdapter mFrom is null.");
        }
